...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(pairs, strat_labels)):
        ...
        for alpha in alphas:
            for beta in betas:
                ...
                for thresh in np.arange(0.0, 1.01, 0.01):
                    ...
                    closeness = abs(alpha - 0.8) + abs(beta - 0.7)
                    ...
                    if f1_val > best_sw_f1:
                        ...
                        if prec_val > best_sw_val_prec:
                            ...
                            current_best_closeness = abs(best_sw_config[0] - 0.8) + abs(best_sw_config[1] - 0.7)
        ...
        print(f"Fold {fold+1} Best Config: alpha={best_alpha:.1f}, beta={best_beta:.1f}, thresh={best_thresh:.2f} (Val F1={best_sw_f1:.4f})")
    ...
    hyperparam_dist = {
        ...
        "beta": f"{np.mean(betas):.2f} ± {np.std(betas):.4f}",
